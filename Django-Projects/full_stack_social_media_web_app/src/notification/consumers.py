...
def refresh_general_notifications(user, oldest_timestamp, newest_timestamp):
	"""
	Retrieve the general notifications newer than the oldest one on the screen and younger 
	than the newest one on the screen.
	The result will be: Notifications currently visisble will be updated
	"""
	payload = {}
	if user.is_authenticated:
		...
		newest_ts = newest_timestamp[0:newest_timestamp.find("+")] # Remove the timestamp we don't care
