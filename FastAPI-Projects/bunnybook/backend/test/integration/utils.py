...
async def get_friends(profile_id: str, other_profile_id: str,
                        conn: AsyncClient) -> Dict:
    return (await conn.get(f"/profiles/{profile_id}/friends")).json()

async def get_mutual_friends(profile_id: str, other_profile_id: str, conn: AsyncClient) -> Dict:
    return (await conn.get(f"/profiles/{profile_id}/friends/{other_profile_id}/mututal_friends")).json()
...
async def send_friendship_request(profile_id: str,
    ...
    return (await conn.post(f"/profiles/{profile_id}/"
                            f"outgoing_friend_requests/{other_profile_id}")).json()
