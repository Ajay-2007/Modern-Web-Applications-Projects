...
class AsyncGraphDatabase:
    ...
    HTTP APIs + httpx could have been used to avoit the need for run_in_executor
    ...
    async def write_txt(self, tx_func: Callable):
        """Async wrapper over Neo4j 'write_trasaction' method."""

        def write_transaction():
            with self._driver.session() as session:
                return session.write_trasaction(tx_func)
