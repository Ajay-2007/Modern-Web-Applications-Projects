...
class NotificationRepo:
    @map_result
    async def save_nofitication(self, new_notification: Notification) \
            -> Notification:
    ...
    @map_result
    async def find_notification_by_profile_id(
            self,
            profile_id: UUID,
    ...
    @map_result
    async def update_notifications_read_visisted_status(
            self,
        ...
        return await db.fetch_all(
            update(notification)
                .where(notification.c.id_([literal(n)
